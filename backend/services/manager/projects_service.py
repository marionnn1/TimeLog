...
def get_all_projects_data():
    # SOLUCIÓN PROBLEMA N+1: joinedload trae todos los datos en UNA consulta
    proyectos_db = Projects.query.filter(Projects.fecha_desactivacion.is_(None))\
        .options(joinedload(Projects.cliente), joinedload(Projects.asignaciones).joinedload(Assignments.usuario))\
        .all()
...
def save_client(data):
    ...
    if not nombre: raise APIError("El nombre del cliente es obligatorio", 400)

    existente = Clients.query.filter_by(nombre=nombre).first()
    if existente: raise APIError("Este cliente ya existe en la base de datos", 400)
...
def update_client(client_id, data):
    ...
    if not cliente: raise APIError("Cliente no encontrado", 404)
...
def delete_client(client_id):
    ...
    if proyectos_activos > 0:
        raise APIError("No puedes eliminar un cliente que tiene proyectos asignados", 400)
...
def save_project(data, project_id=None):
    ...
    if project_id:
        ...
        if not proyecto: raise APIError("Proyecto no encontrado", 404)
    ...
    db.session.commit()
    return {"message": "Proyecto guardado correctamente"}
...
def assign_user(proyecto_id, usuario_id):
    asignacion_existente = Assignments.query.filter_by(proyecto_id=proyecto_id, usuario_id=usuario_id, activo=True).first()
    if asignacion_existente: raise APIError("El usuario ya está en este proyecto", 400)
...
def unassign_user(proyecto_id, usuario_id):
    ...
    if not asignacion: raise APIError("El usuario no está activo en este proyecto", 404)

    asignacion.activo = False
    asignacion.fecha_desactivacion = datetime.utcnow()
    db.session.commit()
    return {"message": "Usuario quitado del proyecto"}