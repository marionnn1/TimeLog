...
def get_closing_audit(mes):
    ...
    imputaciones_mes = TimeEntries.query.filter(
        extract("year", TimeEntries.fecha) == anio, extract("month", TimeEntries.fecha) == mes_num, TimeEntries.horas > 0,
    ).all()
    ausencias_mes = Absences.query.filter(
        extract("year", Absences.fecha) == anio, extract("month", Absences.fecha) == mes_num,
    ).all()

    datos_usuarios = {u.id: {"horas": 0.0, "dias_imputados": set(), "dias_ausencias": set(), "proyectos": {}} for u in usuarios}
    ...
    for u in usuarios:
        ...
        for day in range(1, num_days + 1):
            ...
            if current_date.weekday() < 5: 
                ...
                if day not in datos["dias_imputados"] and day not in datos["dias_ausencias"]:
                    dias_faltantes.append(str(day))

        if len(dias_faltantes) == 0: estado = "completo"
        elif len(dias_faltantes) == dias_laborables_totales and horas_reales == 0: estado = "vacio"
        else: estado = "incompleto"

        usuarios_auditoria.append({
            "id": u.id, "nombre": u.nombre, "rol": u.rol, "horasReales": horas_reales,
            "estado": estado, "diasFaltantes": dias_faltantes, "desgloseProyectos": desglose_proyectos
        })
