...
def get_analytics_data(mes):
    ...
    for p in proyectos_query:
        ...
        proyectos_stats.append({
            "nombre": p.nombre, "horas": horas, "color": "bg-blue-500", "contributors": [],
        })

    empleados_query = (
        db.session.query(
            Users.id, Users.nombre, Users.rol,
    ...
    for u in empleados_query:
        ...
        carga_empleados.append({
            "nombre": u.nombre, "horas": float(u.horas_imputadas or 0),
            "capacidad": capacidad, "rol": u.rol, "avatar": avatar, "trend": "equal",
        })
    ...
    ausencias = [{"date": a.fecha.strftime("%Y-%m-%d") if a.fecha else "", "nombre": a.usuario.nombre if a.usuario else "Desconocido", "type": a.tipo, "userId": a.usuario_id} for a in ausencias_db]
