...
const esJefe = computed(() => rolActual.value === 'manager' || rolActual.value === 'admin')
    ...
    <div class="p-4 border-t border-slate-800 bg-slate-950 shrink-0">
      <div class="flex items-center gap-3 mb-4 pl-1" v-if="currentUser">
        ...
        <div class="overflow-hidden">
          ...
          <p class="text-xs text-slate-400 truncate uppercase">{{ currentUser.rol }}</p>
        </div>
      </div>

      <button @click="handleLogout"
        class="w-full flex items-center gap-3 px-3 py-2 text-slate-400 hover:text-red-400 hover:bg-slate-900 rounded-lg transition text-xs font-bold uppercase tracking-wider group">
        <LogOut class="w-4 h-4 group-hover:text-red-400" />
