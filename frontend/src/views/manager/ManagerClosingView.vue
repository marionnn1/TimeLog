...
// Si es Enero -> Devuelve datos con errores.
// Si es otro mes -> Devuelve limpio.
const getDatosPorMes = (fecha) => {
    // Simulamos que SOLO Enero 2026 tiene datos cargados
    if (fecha === '2026-01') {
        return [
            ...
            { id: 3, nombre: 'Pedro Sola', rol: 'Junior', horasReales: 80, horasTeoricas: 160, diasFaltantes: ['02-Ene', '03-Ene', '... (+10)'], estado: 'incompleto' },
            { id: 4, nombre: 'Laura G.', rol: 'Manager', horasReales: 168, horasTeoricas: 160, diasFaltantes: [], estado: 'completo' }
        ]
    } else {
        // Mes nuevo o vacío: No mostramos errores, simplemente está a cero.
        return [
...
// --- WATCHER: Escucha cambios de fecha ---
watch(fechaCierre, (nuevoMes) => {
    // Reseteamos el estado de cierre (suponemos que meses nuevos están abiertos)
...
const puedeCerrarMes = computed(() => {
    // No se puede cerrar si hay incompletos.
    // Si está 'vacio', técnicamente se podría cerrar (cerrar en blanco), pero asumiremos que sí para no bloquear.
...
const resumenEstado = computed(() => {
    ...
    return { 
        ...
        incompletos, 
        vacios,
        completos: total - incompletos - vacios 
    }
})
...
const ejecutarCierre = () => {
    ...
    // Si está todo vacío, avisamos
    if (resumenEstado.value.vacios === resumenEstado.value.total) {
        if(!confirm("⚠️ El mes parece estar vacío (nadie ha imputado). ¿Cerrar igualmente?")) return;
    }
...
const reabrirMes = () => {
    if(confirm("¿Reabrir el mes?")) {
        mesCerrado.value = false
    }
}

const exportarExcel = () => {
    alert("Descargando informe de auditoría...")
}
  ...
  <div class="h-full flex flex-col font-sans bg-gray-50 p-6 gap-6 overflow-hidden">
    ...
    <div class="flex justify-between items-end shrink-0">
        <div>
            ...
            <p class="subtitle">Selecciona el mes y revisa que todo el equipo haya cumplido.</p>
        </div>
        ...
        <div class="flex items-center gap-4">
            <div class="flex items-center bg-white border border-gray-200 rounded-lg px-3 py-2 shadow-sm">
                ...
                <input type="month" v-model="fechaCierre" class="text-sm font-bold text-dark outline-none bg-transparent">
            </div>
            
            <button @click="exportarExcel" class="btn-secondary flex items-center gap-2">
                <FileDown class="w-4 h-4" /> Exportar Auditoría
            </button>
        </div>
    </div>
    ...
    <div class="card bg-slate-900 text-white p-4 flex justify-between items-center shadow-lg shrink-0 transition-colors duration-300"
        ...
        <div v-else>
            <button @click="reabrirMes" class="px-6 py-2.5 rounded-lg font-bold text-sm bg-white/10 hover:bg-white/20 text-white flex items-center gap-2 transition">
                    ...
                    <tr v-for="user in usuariosFiltrados" :key="user.id" class="hover:bg-slate-50 transition group">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-col items-center gap-1">
                                <span class="font-mono font-bold" :class="user.horasReales === 0 ? 'text-gray-300' : 'text-dark'">
                                    {{ user.horasReales }} <span class="text-gray-300">/ {{ user.horasTeoricas }}</span>
                                </span>
                        ...
                        <td class="px-6 py-4 text-center">
                            ...
                            <span v-else 
                                  class="badge bg-gray-50 text-gray-400 border-gray-200">
                                Sin actividad
                            </span>
                        </td>

                        <td class="px-6 py-4 text-right" v-if="!mesCerrado">
                            <button v-if="user.estado === 'incompleto'" 
                                    @click="notificarUsuario(user.nombre)"
                                    class="text-xs font-bold text-blue-600 hover:text-blue-800 hover:bg-blue-50 px-3 py-1.5 rounded transition border border-transparent hover:border-blue-100 ml-auto flex items-center gap-2">
                                Notificar
                            </button>
