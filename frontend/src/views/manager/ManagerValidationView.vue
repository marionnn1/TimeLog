...
<template>
    <div class="h-full flex flex-col font-sans bg-gray-50 p-6 gap-6 overflow-y-auto relative">
        ...
        <div v-if="alertasVacaciones.length > 0"
            class="bg-rose-50 border border-rose-200 p-4 rounded-xl shadow-sm shrink-0">
            ...
            <p class="text-sm text-rose-600 mb-4">Se ha detectado que 3 o más usuarios han solicitado vacaciones para
                los mismos días. Este es el orden en el que lo solicitaron:</p>

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                <div v-for="alerta in alertasVacaciones" :key="alerta.fecha"
                    class="bg-white border border-rose-200 p-3 rounded-lg shadow-sm">
                    <div class="flex items-center gap-2 font-bold text-rose-700 mb-3 border-b border-rose-100 pb-2">
                        ...
                        <span class="text-[10px] bg-rose-100 px-2 py-0.5 rounded-full ml-auto">{{ alerta.total }}
                            personas</span>
                    </div>
                    <div class="flex flex-col gap-1.5">
                        <div v-for="(user, idx) in alerta.usuarios" :key="idx" class="text-xs flex items-center gap-2"
                            :class="idx === alerta.usuarios.length - 1 ? 'text-rose-700 font-bold' : 'text-slate-600'">
                            <span
                                class="w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-black border shrink-0"
                            ...
                            <span v-if="idx === alerta.usuarios.length - 1"
                                class="text-[8px] uppercase bg-rose-100 text-rose-600 px-1.5 py-0.5 rounded ml-auto shrink-0 border border-rose-200">
                                Último
                            </span>
                        </div>
                    </div>
            ...
            <div v-for="solicitud in solicitudes" :key="solicitud.id"
                ...
                <div class="p-5 bg-slate-50 border-r border-gray-100 w-full md:w-64 flex flex-col gap-3 shrink-0">
                    <div class="flex items-center gap-3">
                        <img v-if="solicitud.foto" :src="'data:image/jpeg;base64,' + solicitud.foto" alt="Avatar"
                            class="w-10 h-10 rounded-full object-cover shadow-sm shrink-0" />
                        <div v-else
                            class="w-10 h-10 rounded-full bg-slate-200 text-slate-600 flex items-center justify-center font-bold shrink-0">
                            {{ solicitud.avatar }}
                        </div>
                        <div>
                ...
                <div class="p-5 flex-1 flex flex-col justify-center gap-3">
                    <div class="flex items-center gap-2">
                        <span
                            class="text-[10px] font-bold uppercase bg-blue-50 text-blue-700 px-2 py-0.5 rounded border border-blue-100">
                            {{ solicitud.cliente }}
                        </span>
                    ...
                    <div
                        class="bg-amber-50 p-3 rounded-lg border border-amber-100 text-sm text-amber-900 flex gap-3 items-start">
                ...
                <div
                    class="p-5 flex flex-col justify-center gap-3 w-full md:w-56 shrink-0 bg-white border-l border-gray-100">
            ...
            <div
                ...
                <div v-if="solicitudSeleccionada" class="p-6 space-y-4">
                    <div class="bg-gray-50 p-3 rounded border border-gray-200 text-sm space-y-1">
                        <p><span class="font-bold text-gray-500 w-20 inline-block">Usuario:</span> {{
                            solicitudSeleccionada.usuario }}</p>
                        <p><span class="font-bold text-gray-500 w-20 inline-block">Proyecto:</span> {{
                            solicitudSeleccionada.proyecto }}</p>
                        <p><span class="font-bold text-gray-500 w-20 inline-block">Fecha:</span> {{
                            solicitudSeleccionada.fecha }}</p>
                    </div>
        ...
        <ConfirmModal :show="confirmState.show" :title="confirmState.title" :message="confirmState.message"
            :type="confirmState.type" :inputMode="confirmState.inputMode" inputPlaceholder="Motivo del rechazo..."
            @confirm="ejecutarConfirmacion" @cancel="confirmState.show = false" />
