    ...
    <div class="h-full flex flex-col font-sans bg-gray-50 p-6 gap-6 overflow-hidden relative">
        ...
        <div class="flex justify-between items-center shrink-0">
            ...
            <button @click="abrirCrear"
                class="btn-primary flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition font-medium shadow-md">
        ...
        <div class="card py-3 shrink-0">
            <div class="relative w-full max-w-sm">
                ...
                <input v-model="busqueda" type="text" placeholder="Buscar por nombre o Azure ID..."
                    class="input-std pl-10 border border-gray-300 rounded-lg w-full py-2 focus:ring-2 focus:ring-blue-500 outline-none">
            </div>
        </div>

        <div
            class="card flex-1 p-0 overflow-hidden relative min-h-[200px] bg-white border border-gray-200 rounded-xl shadow-sm flex flex-col">
            <div v-if="cargando"
            ...
            <div class="flex-1 overflow-y-auto scrollbar-thin">
                <table class="w-full text-left relative">
                    <thead
                        class="bg-gray-50 text-xs uppercase text-gray-500 border-b border-gray-200 sticky top-0 z-10 shadow-sm">
                        <tr>
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm">
                        ...
                        <tr v-for="user in usuariosFiltrados" :key="user.id" class="hover:bg-slate-50 transition"
                            :class="{ 'opacity-50': user.activo === 0 || user.activo === false }">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div v-else
                                        class="w-9 h-9 rounded-full bg-slate-200 flex items-center justify-center text-xs font-bold text-slate-600 shrink-0">
                                    ...
                                    <div>
                                        <div class="flex items-center gap-2">
                                            <p class="font-bold text-slate-800"
                                                :class="{ 'line-through text-slate-400': user.activo === 0 || user.activo === false }">
                                                {{ user.nombre }}</p>
                                            <span v-if="user.activo === 0 || user.activo === false"
                                                class="bg-red-100 text-red-600 border border-red-200 text-[10px] px-2 py-0.5 rounded-full font-bold uppercase">Inactivo</span>
                                        </div>
                            ...
                            <td class="px-6 py-4">
                                <span :class="obtenerEstiloRol(user.rol)"
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <button @click="abrirEditar(user)"
                                        class="p-2 text-gray-400 hover:text-blue-500 transition rounded-lg hover:bg-blue-50"
                                        title="Editar">
                                        <Pencil class="w-4 h-4" />
                                    </button>
                                    <button @click="solicitarAccion(user.id, 'eliminar')"
        ...
        <div v-if="mostrarModal"
            ...
            <div
                class="bg-white rounded-2xl w-full max-w-md space-y-5 shadow-2xl p-6 border border-gray-100 animate-in zoom-in-95">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input v-model="formulario.email"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 outline-none">
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select v-model="formulario.sede"
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 outline-none">
                    ...
                    <div v-if="esEdicion"
                        class="p-4 bg-gray-50 rounded-lg border border-gray-200 mt-2 flex items-center justify-between">
                        <div>
                            <p class="text-sm font-bold text-slate-700">Acceso al Sistema</p>
                            <p class="text-xs text-gray-500">Estado: <span class="font-bold"
                                    :class="(formulario.activo === 1 || formulario.activo === true) ? 'text-emerald-600' : 'text-red-600'">{{
                                        (formulario.activo === 1 || formulario.activo === true) ? 'Activo' : 'Inactivo'
                                    }}</span></p>
                        </div>
                        <button @click.prevent="solicitarAccion(formulario.id, 'toggle')"
                            ...
                            {{ (formulario.activo === 1 || formulario.activo === true) ? 'Desactivar Acceso' :
                            'Reactivar Acceso' }}
                        </button>
                    </div>
                </div>
                ...
                <div class="flex gap-3 pt-2">
                    <button @click="mostrarModal = false" :disabled="isSubmitting"
                        class="flex-1 py-2 border border-gray-300 text-slate-600 rounded-lg font-bold hover:bg-gray-50 transition disabled:opacity-50">Cancelar</button>

                    <button @click="guardar" :disabled="isSubmitting"
                        ...
                        <svg v-if="isSubmitting" class="animate-spin h-4 w-4 text-white"
        ...
        <ConfirmModal :show="confirmacion.show" :title="confirmacion.title" :message="confirmacion.message"
            :type="confirmacion.type" :isLoading="isConfirming" @confirm="confirmarAccion"
