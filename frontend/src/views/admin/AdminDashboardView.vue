...
const announcement = ref({ title: '', message: '' })
...
const exportCSV = async () => {
    try {
        ...
        if (json.status === 'success' || res.ok) {
            ...
            const headers = ['ID', 'Fecha', 'Actor', 'Acción', 'Gravedad', 'Detalle']
            ...
            const contenidoCSV = [headers.join(','), ...rows.map(f => f.join(','))].join('\n')
...
const sendAnnouncement = () => {
    console.log("Enviando comunicado:", announcement.value)
    showAnnouncementModal.value = false
    announcement.value = { title: '', message: '' }
    alert("Comunicado enviado al equipo.")
}

const executeAction = (actionId) => {
    if (actionId === 'comunicado') {
        showAnnouncementModal.value = true
    } else if (actionId === 'exportar') {
        exportCSV()
    } else if (actionId === 'tickets') {
...
<template>
  <div class="h-full p-6 bg-gray-50 flex flex-col gap-6 font-sans overflow-y-auto relative">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div @click="router.push('/admin/users')" class="cursor-pointer bg-white rounded-xl border border-gray-200 shadow-sm p-5 flex items-center justify-between group hover:border-[#26AA9B] transition">
            <div>
                ...
                <p class="text-3xl font-black text-[#232D4B] mt-1">{{ stats.totalUsers }}</p>
            </div>
        ...
        <div @click="router.push('/admin/projects-manager')" class="cursor-pointer bg-white rounded-xl border border-gray-200 shadow-sm p-5 flex items-center justify-between group hover:border-[#26AA9B] transition">
            <div>
                ...
                <p class="text-3xl font-black text-[#232D4B] mt-1">{{ stats.activeProjects }}</p>
            </div>
        ...
        <div @click="router.push('/admin/tickets')" class="cursor-pointer bg-white rounded-xl border border-amber-200 shadow-sm p-5 flex items-center justify-between group hover:border-amber-400 transition relative overflow-hidden bg-amber-50/30">
            ...
            <div class="z-10">
                ...
                <p class="text-3xl font-black text-amber-600 mt-1">{{ stats.pendingTickets }}</p>
                <p class="text-[10px] font-bold text-gray-400 mt-1 uppercase">de {{ stats.totalTickets }} totales</p>
            </div>
    ...
    <div v-if="showAnnouncementModal" class="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
        <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl animate-in zoom-in-95 flex flex-col p-6 border border-gray-100">
            <div class="flex justify-between items-center border-b border-gray-100 pb-4 mb-4">
                ...
                <button @click="showAnnouncementModal = false"><X class="w-5 h-5 text-gray-400 hover:text-red-500 transition" /></button>
            </div>

            <div class="space-y-4">
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1">Título del aviso</label>
                    <input v-model="announcement.title" type="text" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-[#26AA9B] outline-none" placeholder="Ej: Mantenimiento del servidor el Viernes">
                </div>
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1">Mensaje</label>
                    <textarea v-model="announcement.message" rows="4" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-[#26AA9B] outline-none resize-none" placeholder="Escribe aquí los detalles..."></textarea>
                </div>
            </div>

            <div class="flex gap-2 mt-6">
                <button @click="showAnnouncementModal = false" class="flex-1 py-2 border border-gray-300 text-slate-600 rounded-lg font-bold hover:bg-gray-50 transition">Cancelar</button>
                <button @click="sendAnnouncement" class="flex-1 py-2 bg-[#26AA9B] text-white rounded-lg font-bold hover:bg-[#209083] shadow-md flex justify-center items-center gap-2">
