<script setup>
    ...
    const exportarCSV = () => {
        ...
        const rows = proyectosOrdenados.map(p => [`"${p.cliente}"`, `"${p.proyecto}"`, p.real, getPorcentaje(p.real)].join(';'))
    ...
    <div class="absolute inset-0 overflow-y-auto bg-slate-50 font-sans p-6 md:p-8 pb-24 text-slate-800">
        ...
        <div class="max-w-[1600px] mx-auto w-full">
            ...
            <div v-else-if="datos" class="space-y-6">
                ...
                <div v-if="Object.keys(clientesAgrupados).length === 0" class="bg-white border border-slate-200 rounded-xl shadow-sm p-12 text-center">
                    ...
                    <p class="text-sm text-slate-500 mt-2">No tienes horas imputadas en {{ meses[mesSeleccionado-1].n }} de {{ anioSeleccionado }}.</p>
                </div>
                    ...
                    <div v-for="(datosCliente, clienteNombre) in clientesAgrupados" :key="clienteNombre" 
                        ...
                        <div class="px-6 py-4 border-b border-slate-100 bg-[#f8fafc] flex justify-between items-center">
                            ...
                            <div class="flex items-center gap-2">
                                <span class="text-xs font-bold text-slate-500 uppercase tracking-widest">Total en cliente:</span>
                                <span class="bg-white text-blue-700 border border-blue-200 font-bold px-3 py-1 rounded-full text-sm shadow-sm">
