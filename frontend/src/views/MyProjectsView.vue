<script setup>
    ...
    const exportarCSV = () => {
        ...
        const rows = datos.value.proyectos.map(p => [`"${p.proyecto}"`, `"${p.cliente}"`, p.real, getPorcentaje(p.real)].join(';'))
            ...
            <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr
                                class="bg-white border-b border-slate-200 text-[10px] uppercase tracking-widest text-slate-400">
                        ...
                        <tbody v-if="datos.proyectos.length > 0" class="divide-y divide-slate-100 text-sm">
                            <tr v-for="p in datos.proyectos" :key="p.proyecto"
                                ...
                                <td class="px-6 py-4 text-right">
                                    <span
                                        class="inline-block px-3 py-1 bg-slate-100 text-slate-700 font-bold rounded border border-slate-200">
                                        {{ p.real }} h
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                        ...
                        <tbody v-else>
                            <tr>
                                <td colspan="3" class="px-6 py-12 text-center">
                                    ...
                                    <p class="text-xs text-slate-400 mt-1">No tienes horas imputadas en {{
                                        meses[mesSeleccionado-1].n }} de {{ anioSeleccionado }}.</p>
                                </td>
                            </tr>
                        </tbody>
                        <tfoot v-if="datos.proyectos.length > 0" class="bg-slate-50/50 border-t border-slate-200">
                            <tr>
                                <td
                                    class="px-6 py-3 text-xs font-bold text-slate-500 uppercase tracking-widest text-right">
                                    Total</td>
                                <td class="px-6 py-3 text-right">
                                    <span class="font-black text-primary text-base">{{ datos.totales.mes_real }}
                                        h</span>
                                </td>
                                <td></td>
                            </tr>
